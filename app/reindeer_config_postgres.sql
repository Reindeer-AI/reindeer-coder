...
7. Environment setup: If there''s a .env.example file, create .env from it (cp .env.example .env) and ask the user if any additional environment variables need to be configured', 'Default system prompt for agents', FALSE, 'Agent')
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = CURRENT_TIMESTAMP;

			...
			if (existsSync(task.outputFile)) {
				...
				try {
					...
					if (planOutput.startsWith('ERROR:')) {
						...
						const planComment = `## 📋 Implementation Plan\n\n${planOutput}\n\n---\n🤖 *Generated by Claude Code* • Ready for human review`;
		...
		for (const task of tasksNeedingAttention) {
			...
			try {
				...
				if (analysis.state === 'agent_done_waiting') {
					...
					comment += `[View Task Terminal →](${env.APP_URL}/tasks/${task.id})\n\n`;
					...
					const comment =
						...
						`${analysis.reasoning}\n\n` +
						`[View Task Terminal →](${env.APP_URL}/tasks/${task.id})\n\n` +
						`---\n🤖 *Claude Code*`;

...
export class SSHFSManager {
  ...
  async mount(taskId: string, options: MountOptions): Promise<string> {
    try {
      ...
      if (isMounted) {
        this.outputChannel.appendLine(`[SSHFS] Path ${localPath} is already mounted, reusing existing mount`);
  ...
  private async checkPrerequisites(): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(`[SSHFS] gcloud version: ${gcloudResult.stdout.split('\n')[0]}`);
    } catch (error) {
    ...
    try {
      ...
      if (platform === 'darwin') {
        ...
        this.outputChannel.appendLine(`  Windows: Install from https://github.com/winfsp/sshfs-win`);
      }

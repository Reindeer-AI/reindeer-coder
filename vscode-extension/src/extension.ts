...
import { VibeClient, Task } from './api/vibe-client';
import { TaskTreeProvider, TaskTreeItem } from './views/task-tree-provider';
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (!auth0ClientId) {
    const errorMsg = 'Auth0 Client ID is not configured. Please set vibeCoding.auth0ClientId in settings.';
    outputChannel.appendLine(`\n[ERROR] ${errorMsg}`);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('vibeCoding.connectTerminalOnly', async (item: TaskTreeItem) => {
  ...
  try {
    ...
    if (tasks.length > 0) {
      ...
      tasks.forEach((task, i) => {
        const desc = task.task_description ? task.task_description.split('\n')[0].substring(0, 40) : '(no description)';
        outputChannel.appendLine(`  ${i + 1}. ${task.id.substring(0, 8)} - ${task.status} - ${desc}`);
        outputChannel.appendLine(`     VM: ${task.vm_name || 'not assigned'} (${task.vm_zone || 'no zone'})`);
        outputChannel.appendLine(`     Repo: ${task.repository || 'not set'}`);
        outputChannel.appendLine(`     Description type: ${typeof task.task_description}, value: ${task.task_description ? 'present' : 'null/undefined'}`);
      });
    }
 ...
 */
async function connectToTask(taskId: string, gcpProject: string, mountBasePath: string): Promise<void> {
  try {
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        const expandedBasePath = mountBasePath.replace(/^~/, require('os').homedir());
        const localPath = require('path').join(expandedBasePath, folderName);
        ...
        vscode.window.showInformationMessage(`Connected to ${folderName}. Use Terminal > Run Task > "Connect to Vibe Session" to open the remote terminal.`);
      }
    );
  } catch (error) {
